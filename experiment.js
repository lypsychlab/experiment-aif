...
const explanation = {
    ...
    on_load: function() {
        ...
        var current_explain_page = 1

        // Handle updating the grid based on explanation page
        ...
        const callback = function(mutationsList, observer) {
            if (document.querySelector(".jspsych-instructions-pagenum")) {
                let page_num = parseInt(document.querySelector(".jspsych-instructions-pagenum").textContent[5])

                if (page_num != current_explain_page) {
                    setTimeout(function () {
                        update_explanation(explanation_order, page_num - 1)
                    }, 1)

                    current_explain_page = page_num
                }
            } else {
                // Stop observing
                observer.disconnect()
